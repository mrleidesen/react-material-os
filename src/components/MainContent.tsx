...
export default function MainContent() {
  ...
  const computedZIndex = (id: number) => {
    const findIndex =
      state.activeIds.findIndex((activeId) => activeId === id) + 1;
      ...
      {state.icons.map((icon) =>
        state.activeIds.includes(icon.id) ? (
          <WindowApp
            ...
            label={icon.name}
            variant={`${
              state.activeId === icon.id ? "window-draggable--active" : ""
            } ${state.hideIds.includes(icon.id) ? "hidden-important" : ""}`}
            icon={icon.icon}
            style={{
              zIndex: computedZIndex(icon.id),
            }}
          >
            {componentHash[icon.id] || <p>{icon.name}</p>}
          </WindowApp>
        ) : null
      )}
    </div>
